...
const CONFIG = {
    ...
    GRAYSCALE_QUALITY_RANGE: { min: 20, max: 35 },
    DEFAULT_DIMENSIONS: { width: 700, height: 700 },
    DEFAULT_FORMAT: "jpeg"
};
...
async function compress(imagePath, useWebp, grayscale, quality, originalSize) {
    try {
        ...
        const effectiveQuality = grayscale
            ? Math.max(CONFIG.GRAYSCALE_QUALITY_RANGE.min, 
        ...
        logger.debug("Compression started", {
            ...
            effectiveQuality,
            grayscale,
            calculatedDimensions: { width: calculatedWidth, height: calculatedHeight },
            format: finalFormat
        });
        ...
        if (info.size > originalSize) {
            logger.warn("Compression bypassed - output larger than original", {
                originalSize,
                compressedSize: info.size,
                increase: info.size - originalSize,
                percentageIncrease: (((info.size - originalSize) / originalSize) * 100).toFixed(2)
            });
...
async function getImageMetadata(imagePath) {
    try {
        ...
        logger.warn("Metadata read failed, using defaults", { error: error.message });
        return {
...
function createResponse(output, format, size, bytesSaved, status, originalSize = null) {
    const headers = {
